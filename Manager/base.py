...
class Manager:
    def __init__(self, **kwargs):
        ...
        if point_list:
            for i in range(len(point_list)):
                print(point_list[i].x, point_list[i].y)
                self.canvas.create_rectangle(
                    point_list[i].x * 10 + 5,
                    point_list[i].y * 10 + 5,
                    point_list[i].x * 10 + 12,
                    point_list[i].y * 10 + 12,
                    fill="red")

        if point_list:
            self.tick_with_init(point_list)
        else:
            self.tick_random()
        self.root.mainloop()
    ...
    def tick_with_init(self, point_list):
        self.canvas.create_rectangle(5+self.c*10,
                                     5+self.c*10,
                                     5+self.c*10+7,
                                     5+self.c*10+7,
